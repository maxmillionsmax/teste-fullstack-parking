    ...
    public class ClientesController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] ClienteCreateDto dto)
        {
            ...
            if (dto.Mensalista && dto.ValorMensalidade == null)
                return BadRequest("Mensalista precisa ter o valor da mensalidade informado.");

            var existe = await _db.Clientes.AnyAsync(c => c.Nome == dto.Nome && c.Telefone == dto.Telefone);
            if (existe) return Conflict("Já existe outro cliente com o mesmo Nome e Telefone.");
        ...
        public async Task<IActionResult> Update(Guid id, [FromBody] ClienteUpdateDto dto)
        {
            ...
            var existeOutro = await _db.Clientes.AnyAsync(x => x.Id != id && x.Nome == dto.Nome && x.Telefone == dto.Telefone);
            if (existeOutro)
                return Conflict("Já existe outro cliente com o mesmo Nome e Telefone.");
